...
export const useGetMintEvents = (coin1: string, coin2: string) => {
  
  const fetchMintEvents = async (): Promise<MintEventsResponse> => {
    const endpoint = `/liquidity/get_mint_events/${coin1}/${coin2}`;
  ...
  return useQuery(['mintEvents', coin1, coin2], fetchMintEvents);
};
