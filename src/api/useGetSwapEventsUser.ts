...
export const useGetSwapEventsUser = (coin1: string, coin2: string) => {

  const fetchSwapEvents = async (): Promise<SwapEventsResponse> => {
    const endpoint = `/liquidity/get_swap_events_user/${coin1}/${coin2}`;
  ...
  return useQuery(['swapEventsUser', coin1, coin2, 'user'], fetchSwapEvents);
};
